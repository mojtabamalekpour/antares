...
void GetQueue(Ptr<SimpleDCTCPQueue> q)
{
	...
	Simulator::Schedule(Seconds(0.01),&GetQueue,q);
}
...
void Build_Topology()
{
	...
	PointToPointHelper p2p_b;
		p2p_b.SetDeviceAttribute	("DataRate",  StringValue (LinkSpeed));
		p2p_b.SetChannelAttribute	("Delay",     StringValue (LinkDelay));
		queueType = "ns3::DropTailQueue";
		n1 = "Mode";
		v1 = Create<EnumValue> (DropTailQueue::QUEUE_MODE_BYTES);
		n2 = "MaxBytes";
		v2 = Create<UintegerValue> (queueBytes);
		n3 = "";
		v3 = Create<EmptyAttributeValue> ();
		if(dctcp) {
			n3 = "Th";
	...
	for (uint32_t i=10;i<20;i++){
		devices = p2p.Install (nEnd[i],nLeaf[1]);
		interfaces = ipv4.Assign(devices);
		ipv4.NewNetwork();
	}


	devices = p2p_b.Install (nLeaf[0],nLeaf[1]);
	interfaces = ipv4.Assign(devices);
	ipv4.NewNetwork();
...
void SetupServer(Ptr<Node> rnode)
{
	...
	for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
		...
		if(!dctcp){
			...
			Ptr<DCTCPQueue> dtqueue = queue->GetObject<DCTCPQueue>();
			set_successful &= dtqueue->SetAttributeFailSafe("Mode",      EnumValue(DropTailQueue::QUEUE_MODE_BYTES));
			set_successful &= dtqueue->SetAttributeFailSafe("MaxBytes",  UintegerValue(4 * 1024 * 1024)); //4 MB typical to accomodate
			set_successful &= dtqueue->SetAttributeFailSafe("Th",      UintegerValue(4 * 1024 * 1024));
			//Simulator::Schedule(Seconds(0.01),&GetQueue,dtqueue);
