...
uint32_t
PointToPointNetDevice::combine(uint32_t a, uint32_t b) {
uint128_t times = 1;
   while (times <= b)
      times *= 10;
   return a*times + b;
  ...
  if (m_txMachineState == READY) 
    {
	  ...
	  Ipv4Header iph;
	  TcpHeader th;
	  copy->RemoveHeader (hed);
	  copy->RemoveHeader (iph);
	  copy->RemoveHeader (th);

	  uint32_t a=iph.GetSource().Get();
	  uint32_t b=iph.GetDestination().Get();
	  uint32_t c=th.GetSourcePort();
	  uint32_t d=th.GetDestinationPort();

	  uint32_t flowID = combine(a ,c) + combine(d ,b);

	  MyTag tag;

	  tag.SetSimpleValue (flowID);
