...
BulkSendApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BulkSendApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
   .AddAttribute ("FlowID", "Priority of the requests",
					  UintegerValue(0),
					  MakeUintegerAccessor (&BulkSendApplication::m_flowid),
					  MakeUintegerChecker<uint32_t> ())
  .AddAttribute ("TenantID", "Priority of the requests",
					 UintegerValue(0),
					 MakeUintegerAccessor (&BulkSendApplication::m_tenantid),
					 MakeUintegerChecker<uint32_t> ())
  ...
  if (!m_socket)
    {
      ...
      tcpSock->SetAttribute("FlowID", UintegerValue (m_flowid));
      tcpSock->SetAttribute("TenantID", UintegerValue (m_tenantid));
